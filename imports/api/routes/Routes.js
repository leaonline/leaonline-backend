import {
  createLoggedinTrigger,
  createLoginTrigger,
  createNotFoundTrigger,
} from './triggers'
...
Routes.settings = {
  path: (appName = ':appName', contextName = ':contextName') =>
    `/settings/${appName}/${contextName}`,
...
Routes.fallback = {
  path: () => '*',
  label: 'pages.redirecting.title',
  triggersEnter: () => [createNotFoundTrigger(Routes.notFound)],
  async load() {
    return true
  },
  target: null,
