...
export const createAddFieldsToQuery =
  (fieldsResolver, { skipUndefined = true } = {}) =>
  (query, fields) => {
    ...
    fieldsArray.forEach((fieldDefinition) => {
      ...
      if (fieldType === 'string') {
        ...
        if (type === 'document') {
          ...
          throw new TypeError(
            `[addFieldsToQuery]: fieldType object - subtype [${type}] not yet implemented!`,
          )
        }
      } else {
        throw new TypeError(
          `[addFieldsToQuery]: fieldType ${fieldType} not yet implemented!`,
        )
      }
    })
  }
