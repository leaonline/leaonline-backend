...
export const loadDocumentsWithDependencies = ({
  instance,
  config,
  logDebug = defaultLog,
  onSubscribed,
  ...
  if (config.methods.getAll) {
    callMethod({
      ...
      success: (allDocuments = {}) => {
        ...
        Object.entries(allDocuments).forEach(([collectionName, documents]) => {
          ...
          if (!collection) {
            throw new Error(
              'Collection name is required to upsert docs. Did you only returned docs without collection name?',
            )
          }
        ...
        if (instance) {
          const count = instance.mainCollection
            .find({}, { reactive: false })
            .count()
    ...
    defaultNotifications(
      new Error(
        `[${config.name}]: expected one of the following methods: [getAll, getOne, get], got none.`,
      ),
    )
  }
}
